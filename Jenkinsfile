...
EOF
        ...
        stage('Run Selenium Tests') {
            steps {
                echo 'Running automated Selenium test cases...'
                sh '''
                    echo "=========================================="
                    ...
                    # Ensure application is ready for testing
                    echo "Verifying application readiness..."
                    sleep 5
                    
                    # Check if application is accessible
                    curl -f http://localhost:8080 || {
                        echo "❌ Application not accessible for testing"
                        exit 1
                    }
                    ...
                    python setup_chromedriver.py || echo "ChromeDriver setup attempted"
                    
                    echo "Running 10 comprehensive Selenium test cases..."
                    echo "Test Target: http://localhost:8080"
                    
                    # Run the simplified test suite with detailed output
                    python -m pytest test_simplified.py -v \\
                        ...
                        --capture=no \\
                        || {
                        echo "❌ Some tests failed. Check test report for details."
                        echo "Test report generated at: backend/test_report.html"
                        exit 1
                    }
            ...
            post {
                always {
                    ...
                    archiveArtifacts artifacts: 'backend/test_report.html', fingerprint: true
                    publishHTML([
                        allowMissing: false,
